...
fn canvas_scenario_preserves_domain_fields_while_emitting_intents() {
    ...
    assert_eq!(model.select_at(ScreenPoint { x: 20.0, y: 20.0 }).as_deref(), Some("text"));
    model.set_mode(CanvasMode::Connect);
    assert_eq!(model.pointer_down(ScreenPoint { x: 140.0, y: 20.0 }).as_deref(), Some("file"));
    ...
    model.resize_intent("file", Size { width: 96.0, height: 48.0 });

    let commands = model.take_commands();
    assert!(commands.iter().any(|command| {
        matches!(command, CanvasCommand::Select(Some(id)) if id == "text")
    }));
    assert!(commands.iter().any(|command| {
    ...
    assert!(commands.iter().any(|command| {
        matches!(command, CanvasCommand::Move { id, .. } if id == "text")
    }));
    assert!(commands.iter().any(|command| {
        matches!(command, CanvasCommand::Resize { id, .. } if id == "file")
    }));

    // CanvasModel emits intents and does not mutate the host-owned domain snapshot.
    document = model.document;
    let round_trip = serde_json::from_str::<serde_json::Value>(&document.to_json().unwrap()).unwrap();
    ...
    assert_eq!(document.node("link").unwrap().url.as_deref(), Some("https://example.com"));
    assert_eq!(document.node("group").unwrap().kind, "group");
    assert_eq!(document.edges[0], Edge {
        id: "existing".into(),
        from_node: "text".into(),
        to_node: "file".into(),
        extra: [("futureEdge".into(), serde_json::json!({"kind": "dashed"}))].into_iter().collect(),
        ..Default::default()
    });
    assert!(document.node("text").unwrap().rect().contains(Point { x: 20.0, y: 20.0 }));
}
