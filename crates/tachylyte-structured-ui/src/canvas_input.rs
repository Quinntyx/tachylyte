...
impl ResizeState {
    pub fn new(origin: Size, minimum: Size) -> Self {
        Self {
            origin,
            current: origin,
            minimum,
    ...
    pub fn update(&mut self, current: Size) {
        self.current = Size {
            width: current.width.max(self.minimum.width),
            height: current.height.max(self.minimum.height),
        };
    }
...
pub fn nodes_in_selection<'a>(
    nodes: impl IntoIterator<Item = &'a Node>,
    selection: Rect,
) -> BTreeSet<String> {
    nodes
        .into_iter()
        .filter(|n| contains_rect(selection, n.rect()))
        .map(|n| n.id.clone())
        .collect()
}
...
pub fn apply_selection(
    ...
    clicked: impl IntoIterator<Item = String>,
    additive: bool,
) {
...
pub fn expand_group_selection(nodes: &[Node], selected: &BTreeSet<String>) -> BTreeSet<String> {
    ...
    for n in nodes {
        for key in ["parent", "group"] {
            if let Some(parent) = n.extra.get(key).and_then(|v| v.as_str()) {
                children
                    .entry(parent.into())
                    .or_default()
                    .insert(n.id.clone());
        ...
        if let Some(values) = n.extra.get("children").and_then(|v| v.as_array()) {
            for child in values.iter().filter_map(|v| v.as_str()) {
                children
                    .entry(n.id.clone())
                    .or_default()
                    .insert(child.into());
...
mod tests {
    ...
    #[test]
    fn geometry_and_resize() {
        assert_eq!(
            normalized_selection(Point { x: 4., y: 8. }, Point { x: 1., y: 2. }),
            Rect {
                x: 1.,
                y: 2.,
                width: 3.,
    ...
    #[test]
    fn additive_and_groups() {
        let mut s = BTreeSet::from(["a".into()]);
        apply_selection(&mut s, vec!["b".into()], true);
        assert_eq!(s.len(), 2);
        let mut b = node("b", 0., 0.);
        b.extra.insert("parent".into(), json!("a"));
        let mut c = node("c", 0., 0.);
        c.extra.insert("group".into(), json!("b"));
        assert_eq!(
            expand_group_selection(&[node("a", 0., 0.), b, c], &BTreeSet::from(["a".into()])).len(),
            3
        );
    }
}
