...
impl CanvasHistory {
    pub(crate) fn new(document: CanvasDocument) -> Self {
        Self { document, ..Self::default() }
    }

    pub(crate) fn document(&self) -> &CanvasDocument { &self.document }
    pub(crate) fn can_undo(&self) -> bool { self.undo_depth != 0 }
    pub(crate) fn can_redo(&self) -> bool { self.redo_depth != 0 }
    ...
    pub(crate) fn undo(&mut self) -> bool {
        ...
        if changed { self.undo_depth -= 1; self.redo_depth += 1; }
        changed
    }

    pub(crate) fn redo(&mut self) -> bool {
        let changed = self.history.redo(&mut self.document);
        if changed { self.redo_depth -= 1; self.undo_depth += 1; }
        changed
    }
    ...
    pub(crate) fn create_node(&mut self, node: Node) -> Result<()> {
        self.execute(move |doc| {
            node.validate()?;
            if doc.node(&node.id).is_some() { return Err(Error::Invalid("duplicate node id".into())); }
            doc.nodes.push(node);
            Ok(())
        })
    }
    ...
    pub(crate) fn delete_node(&mut self, id: &str) -> Result<()> {
        ...
        self.execute(move |doc| {
            ...
            if before == doc.nodes.len() { return Err(Error::Invalid("unknown node".into())); }
            doc.edges.retain(|edge| edge.from_node != id && edge.to_node != id);
            Ok(())
        })
    }
    ...
    pub(crate) fn duplicate_node(&mut self, id: &str) -> Result<Node> {
        let source = self.document.node(id).cloned()
            .ok_or_else(|| Error::Invalid("unknown node".into()))?;
        let mut copy = source.clone();
        let mut suffix = 1usize;
        loop {
            let candidate = if suffix == 1 { format!("{}-copy", id) } else { format!("{}-copy-{suffix}", id) };
            if self.document.node(&candidate).is_none() { copy.id = candidate; break; }
            suffix += 1;
        }
...
mod tests {
    ...
    fn node(id: &str) -> Node {
        Node { id: id.into(), kind: "text".into(), x: 0., y: 0., width: 100., height: 50., text: None, file: None, url: None, color: None, extra: [("future".into(), json!(true))].into() }
    }
    ...
    fn crud_is_atomic_and_preserves_unknown_fields() {
        ...
        h.connect_edge(Edge { id: "e".into(), from_node: "a".into(), to_node: copy.id.clone(), ..Default::default() }).unwrap();
