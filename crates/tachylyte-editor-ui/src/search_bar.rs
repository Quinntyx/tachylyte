...
impl SearchBar {
    pub fn new(source: impl Into<String>) -> Self {
        let mut bar = Self { source: source.into(), ..Self::default() };
        bar.recompute();
        bar
    }
    ...
    pub fn results(&self) -> &[ResultSpan] { &self.results }
    pub fn active_result_index(&self) -> Option<usize> { self.active }
    pub fn active_result(&self) -> Option<ResultSpan> { self.active.map(|i| self.results[i].clone()) }
    ...
    pub fn set_case_sensitive(&mut self, value: bool) {
        if self.case_sensitive != value { self.case_sensitive = value; self.recompute(); }
    }

    pub fn next_result(&mut self) -> Option<ResultSpan> {
        if self.results.is_empty() { self.active = None; } else {
    ...
    pub fn replace_current_plan(&self) -> Option<ReplacementPlan> {
        self.active_result().map(|r| ReplacementPlan { span: r.span, replacement: self.replacement.clone() })
    }

    pub fn replace_all_plans(&self) -> Vec<ReplacementPlan> {
        self.results.iter().map(|r| ReplacementPlan {
            span: r.span.clone(), replacement: self.replacement.clone(),
        }).collect()
    }
    ...
    fn recompute(&mut self) {
        ...
        for start in 0..chars.len() {
            if start + query.len() > chars.len() { break; }
            ...
            if matches {
                ...
                let end = if start + query.len() < chars.len() { chars[start + query.len()].0 } else { self.source.len() };
