...
impl EditorIntent {
    ...
    pub fn at(source: &str, offset: usize) -> Option<Self> {
        ...
        scan(source)
            .into_iter()
            .find(|intent| intent.span().start <= offset && offset <= intent.span().end)
    }
    ...
    pub fn span(&self) -> SourceSpan {
        match self {
            ...
            | Self::WikiLink { span, .. }
            | Self::Tag { span, .. }
            | Self::ToggleTask { span, .. }
            | Self::Embed { span, .. }
            | Self::NavigateBlock { span, .. } => *span,
        }
    }
}
    ...
    while i < bytes.len() {
        if source[i..].starts_with("![[") {
            if let Some(end) = source[i + 3..].find("]]") {
                let e = i + 3 + end + 2;
                out.push(EditorIntent::Embed {
                    target: source[i + 3..i + 3 + end].to_owned(),
                    span: SourceSpan::new(i, e),
                });
                i = e;
                continue;
            }
            ...
            if let Some(end) = source[i + 2..].find("]]") {
                ...
                if let Some(block) = target.rfind("#^") {
                    out.push(EditorIntent::NavigateBlock {
                        reference: target[block + 2..].to_owned(),
                        span: SourceSpan::new(i, e),
                    });
                } else {
                    out.push(EditorIntent::WikiLink {
            ...
            if let Some(close) = source[i + 1..].find(']') {
                ...
                if source[after..].starts_with('(') {
                    if let Some(end) = source[after + 1..].find(')') {
                        let e = after + 1 + end + 1;
                        out.push(EditorIntent::Link {
                            destination: source[after + 1..after + 1 + end].trim().to_owned(),
                            span: SourceSpan::new(i, e),
                        });
                        i = e;
                        continue;
        ...
        for (j, c) in content.char_indices() {
            if c == '#' && (j == 0 || content.as_bytes()[j - 1].is_ascii_whitespace()) {
                let end = content[j + 1..]
                    ...
                    .map_or(content.len(), |n| j + 1 + n);
                if end > j + 1 {
                    out.push(EditorIntent::Tag {
                        name: content[j + 1..end].to_owned(),
                        span: SourceSpan::new(line_start + j, line_start + end),
                    });
                }
            }
        }
...
mod tests {
    use super::*;
    #[test]
    fn scans_common_intents() {
        let got = scan("[x](url) [[Page]] ![[img.png]] - [x] done #tag [[N#^b]]");
        assert!(got
            .iter()
            .any(|x| matches!(x, EditorIntent::Link { destination, .. } if destination == "url")));
        assert!(got
            .iter()
            .any(|x| matches!(x, EditorIntent::Embed { target, .. } if target == "img.png")));
        assert!(got
            .iter()
            .any(|x| matches!(x, EditorIntent::ToggleTask { checked: true, .. })));
        assert!(got.iter().any(
            |x| matches!(x, EditorIntent::NavigateBlock { reference, .. } if reference == "b")
        ));
    }
    #[test]
    fn invalid_offsets_are_safe() {
        assert!(EditorIntent::at("é", 1).is_none());
